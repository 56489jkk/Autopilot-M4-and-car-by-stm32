...
static inline void mavlink_msg_optical_flow_send(mavlink_channel_t chan, uint64_t time_usec, uint8_t sensor_id, int16_t flow_x, int16_t flow_y, float flow_comp_m_x, float flow_comp_m_y, uint8_t quality, float ground_distance, float flow_rate_x, float flow_rate_y)
{
    ...
    mavlink_optical_flow_t packet;
    packet.time_usec = time_usec;
    packet.flow_comp_m_x = flow_comp_m_x;      //经过角速度补偿后得到的线速度x m/s
    packet.flow_comp_m_y = flow_comp_m_y;      //经过角速度补偿后得到的线速度y m/s
    packet.ground_distance = ground_distance;  //高度
    packet.flow_x = flow_x;                    //raw_data_x  dpix
    packet.flow_y = flow_y;                    //raw_data_y  dpix
    packet.sensor_id = sensor_id;
    packet.quality = quality;
    packet.flow_rate_x = flow_rate_x;          //rad/s
    packet.flow_rate_y = flow_rate_y;          //rad/s
